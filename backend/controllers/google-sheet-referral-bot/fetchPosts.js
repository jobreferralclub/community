  ...
  while (attempt < maxRetries) {
    try {
      ...
      if (error.response) {
        ...
        console.error(
          `[POST ERROR] Attempt ${attempt} - No response received:`,
          error.message
        );
      } else {
        console.error(
          `[POST ERROR] Attempt ${attempt} - Request setup error:`,
          error.message
        );
      }
      if (attempt < maxRetries) {
        ...
        console.error(
          "[POST ABORT] Max retries reached, skipping this job post."
        );
        return false;
      }
    }
...
const sheetsToProcess = [
  ...
  {
    id: PROGRAM_AND_PROJECT_US_SHEET_ID,
    community: "Program & Project Management - US",
  },
  { id: PRODUCT_US_SHEET_ID, community: "Product Management - US" },
  { id: MARKETING_US_SHEET_ID, community: "Marketing Management - US" },
  {
    id: CATEGORY_AND_VENDOR_US_SHEET_ID,
    community: "Category and Vendor Management - US",
  },
  {
    id: SALES_AND_ACCOUNT_US_SHEET_ID,
    community: "Sales and Account Management - US",
  },
...
export async function generatePostsAll() {
  ...
  for (const sheet of sheetsToProcess) {
    ...
    for (const row of data) {
      ...
      if (!jobId) {
        console.log(
          `[DEBUG] Empty jobId in ${sheet.community}: Row keys=[${Object.keys(
            row
          ).join(", ")}], Raw row=`,
          row
        );
      ...
      if (!isJobIdNewer(jobId, lastJobIdLog[sheet.community])) {
        console.log(
          `[SKIP] Job ID ${jobId} <= last posted Job ID ${
            lastJobIdLog[sheet.community]
          } (${sheet.community})`
        );
        continue;
      }
      ...
      const job_description =
        row["About the Job"] || row["Job Description"] || "";
      ...
      const message = isHiringManagerMissing
        ...
        : `
            ...
            <p>${hiringManagerName} is hiring for <strong>${
            row["Job Title"] || ""
          }</strong> at <strong>${row["Company Name"]}</strong>.</p>
            <p>Refer the 
              <a href="${jobLink}" target="_blank" rel="noopener noreferrer" style="color:#79E708;font-weight:600;text-decoration:underline;">
                Job Description
              </a> 
      ...
      console.log(
        `[CREATE] Creating post for Job ID: ${jobId} in community: ${sheet.community}`
      );
      const success = await createPost(postData);

      console.log(
        `[RESULT] Post for JobID ${jobId} (${sheet.community}) => ${success}`
      );
      ...
      if (success) {
        ...
        console.log(
          `[SAVED] Updated lastJobId for "${sheet.community}" => ${jobId}`
        );
      }
    }

    console.log(
      `[WAIT] Waiting 1 minute before moving to next sheet (${sheet.community})...`
    );
